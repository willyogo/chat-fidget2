...
const personal_sign_1 = require("./personal-sign");
...
describe('personalSign', function () {
    ...
    const helloWorldSignature = '0x90a938f7457df6e8f741264c32697fc52f9a8f867c52dd70713d9d2d472f2e415d9c94148991bbe1f4a1818d1dff09165782749c877f5cf1eff4ef126e55714d1c';
    const helloWorldMessage = `0x${Buffer.from('Hello, world!').toString('hex')}`;
    it('should sign a message', function () {
        expect((0, personal_sign_1.personalSign)({ privateKey, data: helloWorldMessage })).toBe(helloWorldSignature);
    });
    it('should recover the address from a signature', function () {
        const address = (0, util_1.addHexPrefix)((0, util_1.privateToAddress)(privateKey).toString('hex'));
        expect((0, personal_sign_1.recoverPersonalSignature)({
            data: helloWorldMessage,
            signature: helloWorldSignature,
        })).toBe(address);
    });
    it('should recover the public key from a signature', function () {
        const publicKey = (0, util_1.addHexPrefix)((0, util_1.privateToPublic)(privateKey).toString('hex'));
        expect((0, personal_sign_1.extractPublicKey)({
            data: helloWorldMessage,
            signature: helloWorldSignature,
        })).toBe(publicKey);
    });
    it('should sign a message and recover the address of the signer', function () {
        const address = (0, util_1.addHexPrefix)((0, util_1.privateToAddress)(privateKey).toString('hex'));
        const signature = (0, personal_sign_1.personalSign)({
            privateKey,
            data: helloWorldMessage,
        });
        expect((0, personal_sign_1.recoverPersonalSignature)({
    ...
    for (const { testLabel, message, signature, addressHex, key } of testCases) {
        it(`${testLabel}`, function () {
            const signed = (0, personal_sign_1.personalSign)({ privateKey: key, data: message });
            expect(signed).toBe(signature);
            const recovered = (0, personal_sign_1.recoverPersonalSignature)({
                data: message,
                signature,
            });
    ...
    describe('validation', function () {
        describe('personalSign', function () {
            it('should throw if passed null data', function () {
                expect(() => (0, personal_sign_1.personalSign)({
                    privateKey,
                    data: null,
...
//# sourceMappingURL=personal-sign.test.js.map